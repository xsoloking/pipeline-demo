...
pipeline {
    ...
    stages {
        stage('Task') {
            ...
            steps {
                sh '''
                # Check RWX PVC mount status
                df -h
                # Check custom workspace 
                echo "Default workspace is $(pwd)"
                # Task image includes kubectl client.
                docker info
                '''

                // Custom Jenkins shared library: gitCheckout
                gitCheckout subDir: "src", branch: gitRevision, credentialsId: credentialsId, url: gitUrl 
            }
        }
    }
}
